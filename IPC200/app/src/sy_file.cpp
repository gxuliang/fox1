...
static long findfirst(const char* fileName, FileInfo* data)
{
	...
	memcpy(data->name, finddata.name, MAX_PATH - 1);
	data->name[MAX_PATH - 1] = '\0';
...
again:
		if ((dirent = readdir(directory)))
		{
			...
			if(dirent->d_reclen >= MAX_PATH)
			{
				tracef("findnext dir too long : %s\n", dirent->d_name);
				goto again;
			}
...
static int _statfs(const char *path, uint64* userFreeBytes, uint64* totalBytes, uint64* totalFreeBytes)
{
	...
	*totalFreeBytes = 0;
	if (statfs(path, &info) == 0)
	{
		...
		*totalFreeBytes = (uint64)(info.f_bsize) * info.f_bfree;
	}
#endif
	return 0;
}
...
static char fsPaths[fsMaxOptNum][32] = {0};
